...
if __name__ == "__main__":
	...
	try:
		...
		#dr = calculate.date_range(start_date, end_date)
		#dates = list(dr)
	...
	for i in range(delta.days + 1):
		db.connect()
		date = {
			'date' 		: (start_date + td(days=i)).date(), 
			'start_date': (start_date + td(days=i)).date(),
			'end_date' 	: (start_date + td(days=i+1)).date()	
			}
		print date
	
		# Check timesheet for anyone working
		query = "SELECT emp_id, clock_in, clock_out FROM timesheets WHERE `date` between '%(date)s' and date_add('%(date)s', interval 1 day)"
		...
		from transactions where CONVERT_TZ(created_at,'UTC','America/Los_Angeles') between '%(start_date)s' and '%(end_date)s'
